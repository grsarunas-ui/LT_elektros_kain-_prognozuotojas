...
def normalize_lgbm_params(params: dict) -> dict:
    params = dict(params)
...
def make_output_paths(dataset_name: str):
    ...
    return {
        "predictions": processed_dir / f"lgbm_predictions_{dataset_name}.csv",
        "model": models_dir / f"lgbm_model_{dataset_name}.pkl",
        "importance_csv": reports_dir / f"lgbm_feature_importance_{dataset_name}.csv",
        "top_errors_csv": reports_dir / f"lgbm_top_errors_{dataset_name}.csv",
        "summary_json": reports_dir / f"lgbm_summary_{dataset_name}.json",
        "cv_results_csv": reports_dir / f"lgbm_cv_results_{dataset_name}.csv",
    }


def save_feature_importance(model: LGBMRegressor, feature_names, out_path: Path):
...
def tune_lgbm_fast(X: pd.DataFrame, y: pd.Series):
    if len(X) < (N_SPLITS + 1):
        raise ValueError(
    ...
    best_params = normalize_lgbm_params(best_params)
...
def train_lgbm(
    ...
    best_params, cv_results_df = tune_lgbm_fast(X_train, y_train)
